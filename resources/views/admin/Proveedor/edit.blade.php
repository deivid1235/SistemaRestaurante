...
<div class="w-full px-0 sm:px-6 animate-fade-in text-slate-700 space-y-6">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-4 border-b border-slate-100">
        <div class="flex items-center gap-3">
            ...
            <div>
                ...
                <p class="text-xs font-semibold text-slate-400 uppercase tracking-wider">Actualiza la información del distribuidor o aliado comercial</p>
            </div>
        </div>
    </div>
    ...
    <div class="bg-white rounded-2xl border border-slate-100 p-4 sm:p-6 shadow-sm transition-all hover:shadow-md">
        ...
        <form action="{{ route('admin.Proveedor.update', $proveedor->id) }}" method="POST" class="space-y-6">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-5">
                ...
                <div class="space-y-1.5">
                    ...
                    <div class="relative">
                        <select name="tipo_documento" 
                            ...
                            <option value="DNI" {{ $proveedor->tipo_documento == 'DNI' ? 'selected' : '' }}>DNI (Persona Natural)</option>
                            <option value="RUC" {{ $proveedor->tipo_documento == 'RUC' ? 'selected' : '' }}>RUC (Persona Jurídica)</option>
                        </select>
                ...
                <div class="space-y-1.5">
                    ...
                    <input type="number" name="numero" value="{{ $proveedor->numero }}" placeholder="Ej. 20123456789"
                ...
                <div class="space-y-1.5">
                    ...
                    <input type="text" name="razon_social" value="{{ $proveedor->razon_social }}" placeholder="Nombre de la empresa o proveedor"
                        class="w-full bg-slate-50/60 border border-slate-200 text-slate-700 p-2.5 px-3 rounded-xl focus:bg-white focus:border-sky-500 focus:ring-4 focus:ring-sky-500/10 transition-all outline-none font-medium text-sm" required>
                </div>
                ...
                <div class="space-y-1.5">
                    ...
                    <input type="text" name="direccion" value="{{ $proveedor->direccion }}" placeholder="Av. Principal Nro. 123 - Ciudad"
                ...
                <div class="space-y-1.5">
                    ...
                    <input type="number" name="telefono" value="{{ $proveedor->telefono }}" placeholder="Ej. 987654321"
                        class="w-full bg-slate-50/60 border border-slate-200 text-slate-700 p-2.5 px-3 rounded-xl focus:bg-white focus:border-sky-500 focus:ring-4 focus:ring-sky-500/10 transition-all outline-none font-medium text-sm">
                </div>
                ...
                <div class="space-y-1.5">
                    ...
                    <input type="email" name="email" value="{{ $proveedor->email }}" placeholder="proveedor@empresa.com"
                        class="w-full bg-slate-50/60 border border-slate-200 text-slate-700 p-2.5 px-3 rounded-xl focus:bg-white focus:border-sky-500 focus:ring-4 focus:ring-sky-500/10 transition-all outline-none font-medium text-sm">
                </div>
                ...
                <div class="space-y-1.5">
                    ...
                    <input type="text" name="contacto" value="{{ $proveedor->contacto }}" placeholder="Nombre del asesor comercial"
                        class="w-full bg-slate-50/60 border border-slate-200 text-slate-700 p-2.5 px-3 rounded-xl focus:bg-white focus:border-sky-500 focus:ring-4 focus:ring-sky-500/10 transition-all outline-none font-medium text-sm">
                </div>

                <div class="space-y-1.5">
                    <label class="text-xs font-bold text-slate-500 uppercase tracking-wider flex items-center gap-1.5">
                        <i class="fas fa-toggle-on text-slate-400"></i> Estado Actual
                    </label>
                    <div class="relative">
                        <select name="estado" 
                            ...
                            <option value="a" {{ $proveedor->estado == 'a' ? 'selected' : '' }}> Activo (Habilitado para operar)</option>
                            <option value="i" {{ $proveedor->estado == 'i' ? 'selected' : '' }}> Inactivo (Bloqueado temporalmente)</option>
                        </select>
                        <div class="absolute inset-y-0 right-3 flex items-center pointer-events-none text-slate-400 text-xs">
                            <i class="fas fa-chevron-down"></i>
                        </div>
            ...
            <div class="mt-8 pt-5 border-t border-slate-100 flex flex-col-reverse sm:flex-row items-center justify-end gap-3">
                ...
                <button type="submit"
                    ...
                    <i class="fas fa-save text-[11px]"></i> Actualizar Proveedor
                </button>
            </div>
