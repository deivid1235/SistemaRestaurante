...
<div class="relative space-y-6 px-2 md:px-6">
            ...
            <div class="flex items-center gap-6">
                <div class="relative flex items-center justify-center flex-shrink-0">
                    ...
                    <div class="relative w-16 h-16 bg-white/20 backdrop-blur-md rounded-full flex items-center justify-center border border-white/30">
                        <i class="fas fa-utensils text-3xl animate-[pulse_2s_infinite]"></i>
                    </div>
                </div>
                ...
                <div>
                    <span class="bg-white/20 text-[10px] font-bold px-3 py-1 rounded-full uppercase tracking-widest border border-white/20 inline-flex items-center">
                        <i class="fas fa-th-large mr-1.5"></i> Clasificación
                    </span>
                    <h1 class="text-3xl font-black mt-2 tracking-tight">Categorías de Productos</h1>
                    <p class="text-white/80 text-sm font-medium mt-1">
                        Organiza el menú de tu restaurante de manera eficiente.
                    </p>
                </div>
            </div>
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
        @php
            ...
            $activas = $categorias->where('estado', 'a')->count();
            $inactivas = $categorias->where('estado', 'i')->count(); // Nueva variable
            $delivery = $categorias->where('delivery', 1)->count();

            $divisor = $total > 0 ? $total : 1;
            $porcActivas = round(($activas / $divisor) * 100);
            $porcInactivas = round(($inactivas / $divisor) * 100);
        ...
        <div class="group relative bg-white p-4 rounded-2xl shadow-sm border border-slate-100 flex items-center gap-4 transition-all duration-300 hover:shadow-md hover:-translate-y-1 overflow-hidden">
            <div class="w-11 h-11 rounded-xl flex items-center justify-center text-lg shadow-sm transition-transform duration-500 group-hover:scale-110 group-hover:rotate-3"
                style="background: linear-gradient(135deg, var(--primary) 0%, #0096D9 100%); color: white;">
                <i class="fas fa-list"></i>
            </div>
            <div>
                <p class="text-2xl font-black text-slate-800 leading-none">{{ $total }}</p>
                <p class="text-[10px] font-bold text-slate-400 uppercase tracking-tight mt-1">Total Categorías</p>
            </div>
        ...
        <div class="group relative bg-white p-4 rounded-2xl shadow-sm border border-slate-100 transition-all duration-300 hover:shadow-md hover:-translate-y-1 overflow-hidden">
            ...
            <div class="w-full bg-slate-100 h-1.5 rounded-full overflow-hidden">
                <div class="h-full rounded-full transition-all duration-1000 group-hover:animate-pulse" 
                    style="width: {{ $porcActivas }}%; background: linear-gradient(90deg, #10B981, #059669);"></div>
            </div>
        ...
        <div class="group relative bg-white p-4 rounded-2xl shadow-sm border border-slate-100 transition-all duration-300 hover:shadow-md hover:-translate-y-1 overflow-hidden">
            <div class="flex items-center gap-4 mb-3">
                ...
                <div>
                    <p class="text-2xl font-black text-slate-800 leading-none">{{ $inactivas }}</p>
            ...
            <div class="w-full bg-slate-100 h-1.5 rounded-full overflow-hidden">
                <div class="h-full rounded-full transition-all duration-1000 group-hover:animate-pulse" 
                    style="width: {{ $porcInactivas }}%; background: linear-gradient(90deg, #EF4444, #B91C1C);"></div>
            </div>
            <div class="absolute top-0 left-0 w-full h-1 bg-red-500 transform scale-x-0 group-hover:scale-x-100 transition-transform duration-500"></div>
        </div>
    ...
    <div class="flex flex-col md:flex-row md:items-center gap-4 mb-6">
        <div class="flex-1 relative group">
            ...
            <input id="buscador" type="text" placeholder="Buscar categoría..." 
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-6">
        ...
        <div class="categoria group bg-white rounded-3xl border border-slate-200 shadow-sm hover:shadow-xl transition-all duration-300 overflow-hidden"
            data-estado="{{ $cat->estado }}"
            data-nombre="{{ strtolower($cat->descripcion ?? '') }}">
            
            <!-- Contenedor de Imagen (Un poco más alto: h-40) -->
            <div class="relative h-40 overflow-hidden bg-slate-100">
                <div class="absolute top-3 right-3 z-10">
                    <span class="px-3 py-1 rounded-lg text-[9px] font-black uppercase tracking-wider text-white shadow-md"
                        style="background: linear-gradient(135deg, {{ $cat->estado == 'a' ? '#10b981' : '#ef4444' }} 0%, {{ $cat->estado == 'a' ? '#059669' : '#b91c1c' }} 100%);">
                        {{ $cat->estado == 'a' ? 'Activo' : 'Inactivo' }}
                    </span>
                </div>

                @if($cat->imagen && $cat->imagen != 'NULL')
                    <img src="{{ asset('storage/'.$cat->imagen) }}" class="w-full h-full object-cover group-hover:scale-110 transition-transform duration-700">
                @else
            ...
            <div class="p-4">
                <div class="flex justify-between items-start mb-2">
                    <h3 class="text-base font-black text-slate-800 uppercase truncate flex-1 tracking-tight">
                        {{ $cat->descripcion }}
                    </h3>
                    <span class="text-[10px] font-mono font-bold text-slate-400 ml-2 bg-slate-50 px-2 py-0.5 rounded">#{{ $cat->id }}</span>
                </div>

                <div class="flex items-center gap-3 mb-4">
                    <div class="flex items-center gap-1.5 {{ $cat->delivery ? 'text-orange-500' : 'text-blue-500' }}">
                        <i class="fas {{ $cat->delivery ? 'fa-motorcycle' : 'fa-store' }} text-xs"></i>
                        <span class="text-[10px] font-black uppercase tracking-tighter">{{ $cat->delivery ? 'Delivery' : 'Local' }}</span>
                    </div>
                    <span class="text-slate-200">|</span>
                    <span class="text-[10px] font-bold text-slate-500 uppercase">Orden: {{ $cat->orden }}</span>
                </div>

                <!-- Botones de Acción (Más grandes y estilizados) -->
                <div class="flex gap-2">
                    ...
                    style="background: linear-gradient(135deg, var(--primary) 0%, #0096D9 100%);">
                        ...
                        <span class="text-xs font-bold uppercase">Editar</span>
                    </a>
                    
                    <button title="Copiar" class="w-10 h-10 flex items-center justify-center bg-slate-100 text-slate-500 hover:bg-blue-50 hover:text-blue-600 rounded-xl transition-all">
                        <i class="far fa-copy text-sm"></i>
                    </button>

                    <button onclick="abrirEliminar({{ $cat->id }}, '{{ $cat->descripcion }}')"
                        class="w-10 h-10 flex items-center justify-center rounded-xl bg-slate-50 text-red-400 hover:bg-red-600 hover:text-white transition-all shadow-sm">
