...
<div class="w-full px-0 sm:px-6 animate-fade-in text-slate-700 space-y-6">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-4 border-b border-slate-100">
        <div class="flex items-center gap-3">
            ...
            <div>
                <h2 class="text-xl font-black text-slate-800 tracking-tight">Editar Producto</h2>
                <p class="text-xs font-semibold text-slate-400 uppercase tracking-wider">Modifique los datos del producto existente</p>
            </div>
        </div>
    </div>
    ...
    <div class="bg-white rounded-2xl border border-slate-100 p-4 sm:p-6 shadow-sm">
        <form action="{{ route('admin.producto.update', $producto->id) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                ...
                <div class="space-y-1.5">
                    ...
                    <div class="flex h-[42px] rounded-xl border border-slate-200 overflow-hidden focus-within:ring-2 focus-within:ring-sky-500/20 transition-all bg-white">
                        ...
                        <input type="text" name="nombre" value="{{ old('nombre', $producto->nombre) }}" class="w-full bg-transparent px-3 outline-none text-sm text-slate-700" required>
                    </div>
                </div>
                    ...
                    <div class="flex h-[42px] rounded-xl border border-slate-200 overflow-hidden bg-white">
                        ...
                        <select name="id_catg" class="w-full bg-transparent px-3 outline-none text-sm text-slate-700" required>
                            @foreach($categorias as $cat)
                                <option value="{{ $cat->id }}" {{ old('id_catg', $producto->id_catg) == $cat->id ? 'selected' : '' }}>{{ $cat->descripcion }}</option>
                            @endforeach
                        </select>
                    </div>
                </div>
                    ...
                    <div class="flex h-[42px] rounded-xl border border-slate-200 overflow-hidden bg-white">
                        ...
                        <select name="id_areap" class="w-full bg-transparent px-3 outline-none text-sm text-slate-700" required>
                            @foreach($areas as $area)
                                <option value="{{ $area->id }}" {{ old('id_areap', $producto->id_areap) == $area->id ? 'selected' : '' }}>{{ $area->nombre }}</option>
                            @endforeach
                        </select>
                    </div>
                </div>
            ...
            <div class="grid grid-cols-1 lg:grid-cols-4 gap-6 pt-4 border-t border-slate-100">
                <div class="space-y-6 lg:col-span-1">
                    <div class="space-y-1.5">
                        ...
                        <label for="imagen" class="flex flex-col items-center justify-center w-full h-[190px] border-2 border-dashed border-slate-200 rounded-2xl cursor-pointer bg-slate-50/50 hover:bg-white hover:border-sky-400 transition-all text-center p-3 overflow-hidden">
                            @if($producto->imagen)
                            ...
                            @else
                                ...
                                <p class="text-[10px] font-bold text-slate-400 mt-1 uppercase tracking-wider" id="file-text">Cambiar Imagen</p>
                            @endif
                        </label>
                    </div>
                    ...
                    {{-- Estado --}}
                    <div class="space-y-1.5">
                        <label class="text-xs font-bold text-slate-500 uppercase tracking-wider">Estado del Producto</label>
                        <div class="flex items-center">
                            <label class="relative inline-flex items-center cursor-pointer">
                                <input type="checkbox" name="estado" value="a" {{ old('estado', $producto->estado) == 'a' ? 'checked' : '' }} class="sr-only peer">
                                <div class="relative w-14 h-7 bg-slate-300 rounded-full transition-all duration-300 peer-checked:bg-green-600 after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white after:rounded-full after:h-6 after:w-6 after:transition-all peer-checked:after:translate-x-full"></div>
                ...
                <div class="lg:col-span-3 grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div class="space-y-1.5">
                        <label class="text-xs font-bold text-slate-500 uppercase tracking-wider">Notas</label>
                        <div class="rounded-xl border border-slate-200 overflow-hidden h-40">
                            <textarea name="notas" rows="7" class="w-full h-full p-3 outline-none resize-none bg-transparent">{{ old('notas', $producto->notas) }}</textarea>
                        </div>
                    </div>
                    <div class="space-y-1.5">
                        <label class="text-xs font-bold text-slate-500 uppercase tracking-wider">Descripción</label>
                        <div class="rounded-xl border border-slate-200 overflow-hidden h-40">
                            <textarea name="descripcion" rows="7" class="w-full h-full p-3 outline-none resize-none bg-transparent">{{ old('descripcion', $producto->descripcion) }}</textarea>
                        </div>
                    </div>
                </div>
            </div>
