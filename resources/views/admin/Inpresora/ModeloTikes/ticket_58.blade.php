<div style="width: 58mm; max-width: 100%; margin: 0 auto; padding: 4px; font-family: Arial, Helvetica, sans-serif; font-size: 9px; color: #333; background-color: #fff; line-height: 1.3;">
    ...
    <div style="text-align: center; margin-bottom: 8px;">
        <div style="margin-bottom: 3px;">
            ...
            <div style="width: 60px; height: 60px; margin: 0 auto; border-radius: 50%; overflow: hidden; border: 2px solid #eee;">
                <img 
            ...
            <div style="font-weight: bold; font-size: 13px; color: #1f2937; text-transform: uppercase;">
    ...
    <div style="border-top: 1px dashed #bbb; border-bottom: 1px dashed #bbb; text-align: center; padding: 5px 0; margin-bottom: 8px;">
        <div style="font-weight: bold; font-size: 10.5px; color: #000; text-transform: uppercase;">
            {{ $venta->tipoDocumento->nombre ?? 'BOLETA DE VENTA ELECTRÓNICA' }}
        </div>
        <div style="font-size: 9.5px; font-weight: bold; margin-top: 2px; color: #111;">
            {{ $venta->serie_doc ?? 'BA01' }}-{{ $venta->nro_doc ?? '00000133' }}
        </div>
    </div>

    {{-- DATOS DEL CLIENTE / EMISIÓN --}}
    <div style="margin-bottom: 8px; font-size: 8.5px;">
        <table style="width: 100%; border-collapse: collapse;">
            <tr>
                <td style="padding: 1px 0; vertical-align: top; width: 45px; font-weight: bold; color: #111;">Fecha:</td>
            ...
            <tr>
                ...
                <td style="padding: 1px 0; color: #4b5563; text-transform: uppercase;">{{ $venta->cliente->nombres ?? 'SIN CLIENTE' }}</td>
            </tr>
            <tr>
    ...
    <table style="width: 100%; border-collapse: collapse; margin-bottom: 6px; font-size: 8.5px;">
        <thead>
            <tr style="border-bottom: 1px solid #000;">
                <th style="text-align: left; padding: 3px 0; font-weight: bold; width: 12%; color: #000;">CT</th>
                <th style="text-align: left; padding: 3px 2px; font-weight: bold; width: 50%; color: #000;">DESCRIPCIÓN</th>
                <th style="text-align: right; padding: 3px 2px; font-weight: bold; width: 18%; color: #000;">P.U.</th>
                <th style="text-align: right; padding: 3px 0; font-weight: bold; width: 20%; color: #000;">TOTAL</th>
            </tr>
        </thead>
        <tbody>
            ...
            @else
                <tr style="border-bottom: 1px dashed #eee;">
                    ...
                    <td style="padding: 4px 2px; vertical-align: top; text-transform: uppercase; word-break: break-word;">EXTRA ARROZ</td>
    ...
    {{-- TOTALES --}}
    <div style="border-top: 1px dashed #bbb; padding-top: 4px; font-size: 9px;">
        <table style="width: 100%; border-collapse: collapse;">
            ...
            <tr style="border-top: 1px solid #000; font-size: 10px; font-weight: bold;">
                <td style="padding: 4px 0; color: #000;">TOTAL A PAGAR:</td>
                <td style="padding: 4px 0; text-align: right; color: #000;">S/ {{ number_format($venta->total ?? 5.00, 2) }}</td>
            </tr>
        </table>
    </div>

    {{-- TOTAL EN LETRAS --}}
    <div style="margin: 6px 0; font-size: 8px; border-bottom: 1px dashed #bbb; padding-bottom: 6px;">
    ...
    <div style="text-align: center; margin-top: 8px;">
        <div style="text-align: center; margin: 6px 0;">
            @if($venta && isset($venta->id))
                {!! QrCode::size(85)->generate(url('/venta/ticket/' . $venta->id)) !!}
            @else
                <div style="display: inline-block; margin-bottom: 6px;">
                    <svg width="85" height="85" viewBox="0 0 29 29" fill="none" xmlns="http://www.w3.org/2000/svg" style="display: block; margin: 0 auto;">
        ...
        <div style="font-size: 7.5px; color: #4b5563; line-height: 1.2; padding: 0 2px;">
            ...
            @if(!empty($venta->hash_cpe))
                <p style="margin: 1px 0; font-weight: bold; color: #111;">HASH: {{ $venta->hash_cpe }}</p>
            @endif
        </div>

        <div style="margin-top: 8px; font-weight: bold; font-size: 9.5px; color: #000; text-transform: uppercase;">
            ¡Gracias por su preferencia!
        </div>
    </div>

    {{-- BOTONES DE ACCIÓN (Se ocultan automáticamente en la impresión física) --}}
    <div class="no-print" style="text-align: center; margin-top: 20px; display: flex; gap: 6px; justify-content: center; flex-wrap: wrap;">
        
        <button onclick="window.print()" style="padding: 8px 10px; background-color: #1e40af; color: white; border: none; border-radius: 4px; font-weight: bold; font-size: 9px; cursor: pointer; display: flex; align-items: center; gap: 4px;">
        ...
        <a href="https://api.whatsapp.com/send?phone={{ $telefonoCliente }}&text={{ $mensajeWhatsapp }}"
           target="_blank"
           style="padding: 8px 10px; background-color: #065f46; color: white; border-radius: 4px; font-weight: bold; font-size: 9px; text-decoration: none; display: flex; align-items: center; gap: 4px;">
            📱 WhatsApp
        </a>
        ...
        <a href="https://mail.google.com/mail/?view=cm&fs=1&to={{ $correoCliente }}&su={{ $asunto }}&body={{ $cuerpo }}"
           target="_blank"
           style="padding: 8px 10px; background-color: #991b1b; color: white; border-radius: 4px; font-weight: bold; font-size: 9px; text-decoration: none; display: flex; align-items: center; gap: 4px;">
            📧 Gmail
        </a>

        <a href="{{ route('admin.Venta.index') }}"
           style="padding: 8px 10px; background-color: #374151; color: white; border-radius: 4px; font-weight: bold; font-size: 9px; text-decoration: none; display: flex; align-items: center; gap: 4px;">
            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="2.5" stroke="currentColor" style="width: 12px; height: 12px;">
