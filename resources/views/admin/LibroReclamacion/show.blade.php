...
<div class="p-2 md:p-7 bg-gray-100 overflow-hidden overflow-hidden">
    ...
    <div class="rounded-t-3xl p-6 text-white shadow-lg flex flex-col md:flex-row md:items-center justify-between gap-4" 
        ...
        <div class="flex flex-wrap gap-2">
            <a href="{{ route('admin.LibroReclamacion.index') }}" 
               class="px-4 py-2 bg-white/20 hover:bg-white/30 rounded-xl text-xs font-bold border border-white/30 flex items-center gap-2 transition-all active:scale-95">
                <i class="fa fa-arrow-left"></i> Volver
            </a>
            
            <button class="px-4 py-2 bg-gray-900 hover:bg-black text-white rounded-xl text-xs font-bold border border-gray-700 flex items-center gap-2 shadow-md transition-all active:scale-95">
            ...
            <a href="https://mail.google.com/mail/?view=cm&fs=1&to={{ $libroReclamacion->correo }}&su=Respuesta a Reclamo #{{ $libroReclamacion->id }}" 
               target="_blank"
               class="px-4 py-2 bg-blue-600 hover:bg-blue-700 text-white rounded-xl text-xs font-bold flex items-center gap-2 shadow-md transition-all active:scale-95">
                <i class="fa fa-envelope"></i> Gmail
            </a>

            <a href="https://wa.me/51{{ preg_replace('/\s+/', '', $libroReclamacion->telefono) }}" 
               target="_blank"
               class="px-4 py-2 bg-green-500 hover:bg-green-600 text-white rounded-xl text-xs font-bold flex items-center gap-2 shadow-md transition-all active:scale-95">
                <i class="fab fa-whatsapp text-lg"></i> WhatsApp
            </a>
        </div>
    </div>

    <div class="bg-white rounded-b-3xl shadow-2xl p-6 md:p-10 border border-gray-100">
        
        <div class="grid grid-cols-1 md:grid-cols-2 gap-12 md:gap-20 mb-8">
            <div class="space-y-6">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-12 md:gap-20 pt-8 border-t-2 border-gray-50">
            <div class="space-y-4">
                ...
                <div class="bg-gray-50 p-6 rounded-2xl min-h-[150px] border border-gray-100 shadow-inner shadow-gray-200/50">
            ...
            <div class="space-y-4">
                ...
                <div class="bg-blue-50/50 p-6 rounded-2xl border-2 border-blue-100 min-h-[150px] shadow-inner shadow-blue-100/50">
                    <p class="text-[14px] text-blue-900 font-bold leading-relaxed">{{ $libroReclamacion->pedido_concreto }}</p>
                </div>
            </div>
        </div>

        <div class="mt-8 flex flex-wrap justify-center gap-4 pt-4 border-t border-gray-100">
            <a href="{{ route('admin.LibroReclamacion.print', $libroReclamacion->id) }}" target="_blank"
            class="group px-6 py-3 bg-slate-100 hover:bg-slate-900 hover:text-white rounded-2xl flex items-center gap-3 transition-all duration-300 active:scale-95 shadow-sm">

                <i class="fa fa-print text-slate-400 group-hover:text-orange-400"></i>
                <span class="text-[10px] font-black uppercase tracking-widest">
                    Imprimir Documento
                </span>
            </a>
            
           <a href="{{ route('admin.LibroReclamacion.pdf', $libroReclamacion->id) }}"
                class="group px-6 py-3 bg-slate-100 hover:bg-slate-900 hover:text-white rounded-2xl flex items-center gap-3 transition-all duration-300 active:scale-95 shadow-sm">
                <i class="fa fa-file-pdf text-slate-400 group-hover:text-blue-400"></i>
